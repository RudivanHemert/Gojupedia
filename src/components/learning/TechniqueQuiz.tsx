...
const TechniqueQuiz: React.FC<TechniqueQuizProps> = ({ category, title, data }) => {
  const { t, i18n } = useTranslation();
  const [quizQuestions, setQuizQuestions] = useState<QuizQuestion[]>([]);

  useEffect(() => {
    // Pass category and language to generator
    setQuizQuestions(generateQuizQuestions(data, 10, category, i18n.language));
  }, [category, i18n.language, data]); // Re-generate if category, language or data changes
