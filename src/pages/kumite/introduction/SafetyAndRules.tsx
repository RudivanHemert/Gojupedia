...
import { AlertTriangle, Shield, Users, Clock, CheckCircle, AlertCircle, Info } from 'lucide-react';
...
const safetySections = [
  ...
  { id: 'competition-rules', icon: AlertTriangle, color: 'text-orange-600', bgColor: 'bg-orange-50' },
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="p-4 max-w-4xl mx-auto space-y-8">
        ...
        {/* Warning Alert */}
        <div className="flex items-start gap-3 bg-red-50 border-l-4 border-red-400 p-4 rounded">
          <AlertTriangle className="text-red-500 mt-1" size={24} />
          <div>
            <h3 className="font-semibold text-red-800 mb-1">{t('kumite.introduction.safety.warning-title')}</h3>
            <p className="text-red-700 text-sm">{t('kumite.introduction.safety.warning-text')}</p>
          </div>
        </div>
        ...
        <section id="basic-principles">
          <Card className="p-6 border-l-4 border-l-blue-500">
            <div className="flex items-start gap-4">
              <Shield className="text-blue-600 mt-1" size={32} />
        ...
        <section id="equipment">
          <Card className="p-6 border-l-4 border-l-green-500">
            <div className="flex items-start gap-4">
              <Users className="text-green-600 mt-1" size={32} />
              <div className="flex-1">
                    ...
                    <ul className="space-y-2">
                      {['hand-protectors', 'foot-protectors', 'mouth-guard', 'groin-protector'].map(item => (
                        <li key={item} className="flex items-center gap-2 text-sm">
                          ...
                          <span>{t(`kumite.introduction.safety.equipment.required.${item}`)}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                    ...
                    <ul className="space-y-2">
                      {['head-protection', 'chest-protector', 'shin-guards'].map(item => (
                        <li key={item} className="flex items-center gap-2 text-sm">
                          ...
                          <span>{t(`kumite.introduction.safety.equipment.optional.${item}`)}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
        ...
        <section id="competition-rules">
          <Card className="p-6 border-l-4 border-l-orange-500">
            <div className="flex items-start gap-4">
              <AlertTriangle className="text-orange-600 mt-1" size={32} />
              <div className="flex-1">
                      ...
                      {['chukoku', 'keikoku', 'hansoku'].map(item => (
                        <div key={item} className="flex items-center gap-2">
                          ...
                          <span className="text-sm">
                            <strong>{t(`kumite.introduction.safety.penalties.${item}.name`)}</strong>: {t(`kumite.introduction.safety.penalties.${item}.desc`)}
                          </span>
                        </div>
                      ))}
                    </div>
