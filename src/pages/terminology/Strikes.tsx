import React from 'react';
import { useTranslation } from 'react-i18next';
import TheoryHeader from '@/components/theory/TheoryHeader';
import { useParams, useNavigate } from 'react-router-dom';
import { ChevronLeft } from 'lucide-react';

const Strikes = () => {
  const { t } = useTranslation();
  const { strikeId } = useParams();
  const termsObject = t('terminology.sections.strikes-content.terms', { returnObjects: true }) as Record<string, any>;

  // Als er een strikeId is, toon de detailpagina
  if (strikeId) {
    return <StrikeDetail termsObject={termsObject} />;
  }

  // Anders toon de overzichtspagina
  return (
    <div className="min-h-screen bg-white">
      <TheoryHeader 
        title={t('terminology.sections.strikes')}
        description={t('terminology.sections.strikes-content.description')}
        backUrl="/terminology"
      />
      <div className="p-4">
        <div className="w-full max-w-4xl mx-auto">
          <StrikesList termsObject={termsObject} />
        </div>
      </div>
    </div>
  );
};

const StrikesList = ({ termsObject }: { termsObject: Record<string, any> }) => {
  const { t } = useTranslation();
  return (
    <div className="space-y-6">
      <p className="text-muted-foreground">
        {t('terminology.sections.strikes-content.description')}
      </p>
      <div className="grid gap-4">
        {Object.entries(termsObject).map(([key, term]) => (
          <a
            key={key}
            href={`/terminology/strikes/${key}`}
            className="block p-4 border rounded-lg hover:bg-stone-50 transition-colors"
          >
            <div className="flex items-center gap-2 mb-2">
              <span className="text-lg font-japanese">{term.japanese}</span>
              <span className="font-semibold">{term.name}</span>
            </div>
            <p className="text-muted-foreground">{term.english}</p>
          </a>
        ))}
      </div>
    </div>
  );
};

const strikeYoutubeLinks: Record<string, string> = {
  // Goju-Ryu specifieke video's voor slagen
  // 'shuto-uchi': 'https://www.youtube.com/watch?v=...', // Goju-Ryu Shuto Uchi
  // 'uraken-uchi': 'https://www.youtube.com/watch?v=...', // Goju-Ryu Uraken Uchi
  // 'empi-uchi': 'https://www.youtube.com/watch?v=...', // Goju-Ryu Empi Uchi
  // Video's worden alleen toegevoegd als er specifieke Goju-Ryu demonstraties beschikbaar zijn
};

const StrikeDetail = ({ termsObject }: { termsObject: Record<string, any> }) => {
  const { strikeId } = useParams();
  const navigate = useNavigate();
  const term = strikeId ? termsObject[strikeId] : null;
  const videoUrl = strikeId && strikeYoutubeLinks[strikeId] ? strikeYoutubeLinks[strikeId] : null;

  if (!term) {
    return <div className="p-8 text-center text-red-500">Techniek niet gevonden.</div>;
  }

  return (
    <div className="min-h-screen bg-white">
      <div className="p-4 max-w-2xl mx-auto">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 mb-4 text-stone-600 hover:text-stone-900"
        >
          <ChevronLeft className="h-5 w-5" /> Terug
        </button>
        <h1 className="text-3xl font-bold mb-2">{term.name} <span className="text-lg font-japanese">{term.japanese}</span></h1>
        <h2 className="text-lg text-muted-foreground mb-4">{term.english}</h2>
        <p className="mb-4 whitespace-pre-line">{term.details}</p>
        {videoUrl ? (
          <div className="aspect-video mb-4">
            <iframe
              width="100%"
              height="315"
              src={videoUrl.replace('watch?v=', 'embed/')}
              title={term.name}
              frameBorder="0"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowFullScreen
            ></iframe>
          </div>
        ) : (
          <div className="text-muted-foreground italic">Geen video beschikbaar.</div>
        )}
      </div>
    </div>
  );
};

export default Strikes;